...
- 需求发布与浏览：按类别、校区、期望时间和响应状态查看求助需求。
- 需求响应：同学提交交换说明与空闲时段，同一条需求不可重复报名。
- 发起人选定：从候选人中选定一人，按需求时间生成待双方确认的预约，其余响应停止接收；被选同学同时段已有其他预约时选择失败并提示冲突，可改选他人。
- 需求卡状态：需求卡展示我的响应状态（待选定/已入选/未入选）与我的预约状态。
...
- `GET /api/skills`
- `GET /api/needs`：需求列表，含当前用户的响应、预约与候选人
- `GET /api/needs/:id/responses`：发起人查看候选人列表
- `POST /api/needs/:id/responses`：提交响应（交换说明 + 空闲时段，不可重复报名）
- `POST /api/needs/:id/select`：发起人选定候选人，生成待双方确认的预约；时段冲突返回 `SCHEDULE_CONFLICT`
- `GET /api/matches`
